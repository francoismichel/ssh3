...
func mainWithStatusCode() int {
	...
	qconn, status := setupQUICConnection(ctx, *insecure, keyLog, sohDir, pool, knownHostsPath, knownHosts, oidcConfig, options, nil, tty)
	...
	if err != nil {
		log.Error().Msgf("could not establish SOH conversation: %s", err)
		return -1
	}
